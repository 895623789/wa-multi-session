...
export async function generateAutoReply(
    ...
    const defaultOwnerInstruction = `You are a helpful Personal AI Assistant for the business owner. 
    You help with scheduling, management, and basic system queries. 
    Acknowledge the owner's authority. Be helpful and friendly.`;

    const defaultCustomerInstruction = `You are a professional WhatsApp business representative. 
    Your goal is to assist customers, answer questions, and represent the brand with excellence.`;

    const systemPrompt = isOwner
        ? (customInstructions || defaultOwnerInstruction)
        : `${customInstructions || defaultCustomerInstruction}\n\nBUSINESS KNOWLEDGE:\n${businessInfo || "General professional knowledge"}`;
